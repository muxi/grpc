...
static grpc_error *process_send_initial_metadata(
  ...
  *has_compression_algorithm = false;
  ...
  if (initial_metadata->idx.named.grpc_internal_stream_encoding_request != NULL) {
    grpc_mdelem md =
  ...
  } else if (initial_metadata->idx.named.grpc_internal_encoding_request != NULL) {
    grpc_mdelem md = initial_metadata->idx.named.grpc_internal_encoding_request->md;
    ...
    *has_compression_algorithm = true;
     ...
     * default */
    if (channeld->default_stream_compression_algorithm != GRPC_STREAM_COMPRESS_NONE) {
      stream_compression_algorithm = channeld->default_stream_compression_algorithm;
      calld->compression_algorithm = GRPC_COMPRESS_NONE;
    } else {
  ...
  if (stream_compression_algorithm != GRPC_STREAM_COMPRESS_NONE) {
    error = grpc_metadata_batch_add_tail(
        exec_ctx, initial_metadata, &calld->stream_compression_algorithm_storage,
        grpc_stream_compression_encoding_mdelem(calld->compression_algorithm));
  ...
  error = grpc_metadata_batch_add_tail(
      ...
      GRPC_MDELEM_ACCEPT_ENCODING_FOR_ALGORITHMS(channeld->supported_compression_algorithms));

  if (error != GRPC_ERROR_NONE) return error;

  error = grpc_metadata_batch_add_tail(
      exec_ctx, initial_metadata, &calld->accept_stream_encoding_storage,
      GRPC_MDELEM_ACCEPT_STREAM_ENCODING_FOR_ALGORITHMS(channeld->supported_stream_compression_algorithms));
...
static grpc_error *init_channel_elem(grpc_exec_ctx *exec_ctx,
  ...
  channeld->supported_compression_algorithms =
      (((1u << GRPC_COMPRESS_ALGORITHMS_COUNT) - 1) & channeld->enabled_algorithms_bitset) | 1u;

  /* Configuration for stream compression */
  channeld->enabled_stream_compression_algorithms_bitset =
      grpc_channel_args_stream_compression_algorithm_get_states(args->channel_args);
  ...
  channeld->supported_stream_compression_algorithms =
      (((1u << GRPC_STREAM_COMPRESS_ALGORITHMS_COUNT) - 1) &
       channeld->enabled_stream_compression_algorithms_bitset) | 1u;
