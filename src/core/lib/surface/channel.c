...
grpc_channel *grpc_channel_create_with_builder(
  ...
  for (size_t i = 0; i < args->num_args; i++) {
    if (0 == strcmp(args->args[i].key, GRPC_ARG_DEFAULT_AUTHORITY)) {
      ...
      channel->compression_options.default_stream_compression_algorithm.is_set = true;
      channel->compression_options.default_stream_compression_algorithm.algorithm =
          (grpc_stream_compression_algorithm)grpc_channel_arg_get_integer(
              &args->args[i],
              (grpc_integer_options){GRPC_STREAM_COMPRESS_NONE, GRPC_STREAM_COMPRESS_NONE,
      ...
      channel->compression_options.enabled_stream_compression_algorithms_bitset =
          (uint32_t)args->args[i].value.integer |
          0x1; /* always support no compression */
    }
  }
