...
static grpc_stream_compression_algorithm stream_compression_algorithm_for_level_locked(
    grpc_call *call, grpc_stream_compression_level level) {
  return grpc_stream_compression_algorithm_for_level(level,
...
static void set_stream_encodings_accepted_by_peer(grpc_exec_ctx *exec_ctx,
  ...
  /* Always support no compression */
  GPR_BITSET(&call->stream_encodings_accepted_by_peer, GRPC_STREAM_COMPRESS_NONE);
  for (i = 0; i < accept_encoding_parts.count; i++) {
    grpc_slice accept_encoding_entry_slice = accept_encoding_parts.slices[i];
...
static grpc_stream_compression_algorithm decode_stream_compression(grpc_mdelem md) {
  grpc_stream_compression_algorithm algorithm =
...
static void validate_filtered_metadata(grpc_exec_ctx *exec_ctx,
  ...
  if (call->incoming_stream_compression_algorithm != GRPC_STREAM_COMPRESS_NONE) {
    ...
    if (algo >= GRPC_STREAM_COMPRESS_ALGORITHMS_COUNT) {
      gpr_asprintf(&error_msg, "Invalid stream compression algorithm value '%d'.",
                   algo);
    ...
    if (!GPR_BITGET(call->stream_encodings_accepted_by_peer,
      ...
      if (GRPC_TRACER_ON(grpc_compression_trace)) {
        char *algo_name = NULL;
        grpc_stream_compression_algorithm_name(call->incoming_stream_compression_algorithm,
                                        &algo_name);
        gpr_log(GPR_ERROR,
  ...
  for (i = 0; i < nops; i++) {
    ...
    switch (op->op) {
      case GRPC_OP_SEND_INITIAL_METADATA:
        ...
        if (op->data.send_initial_metadata.maybe_stream_compression_level.is_set) {
          effective_compression_level =
              op->data.send_initial_metadata.maybe_stream_compression_level.level;
          level_set = true;
          stream_compression = true;
        } else if (op->data.send_initial_metadata.maybe_compression_level.is_set) {
          effective_compression_level =
          ...
          if (copts.default_stream_compression_level.is_set) {
            level_set = true;
            effective_stream_compression_level = copts.default_stream_compression_level.level;
            stream_compression = true;
        ...
        if (level_set && !call->is_client) {
          if (stream_compression) {
            const grpc_stream_compression_algorithm calgo =
                stream_compression_algorithm_for_level_locked(call,
                                                              effective_stream_compression_level);
            compression_md.key = GRPC_MDSTR_GRPC_INTERNAL_STREAM_ENCODING_REQUEST;
            compression_md.value = grpc_stream_compression_algorithm_slice(calgo);
          } else {
