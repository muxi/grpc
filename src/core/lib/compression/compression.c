...
int grpc_stream_compression_algorithm_name(grpc_stream_compression_algorithm algorithm,
                                           char **name) {
  GRPC_API_TRACE("grpc_stream_compression_algorithm_parse(algorithm=%d, name=%p)", 2,
                 ((int)algorithm, name));
  switch (algorithm) {
...
int grpc_compression_options_is_stream_compression_algorithm_enabled(
    const grpc_compression_options *opts,
    grpc_stream_compression_algorithm algorithm) {
  return GPR_BITGET(opts->enabled_stream_compression_algorithms_bitset, algorithm);
}
...
GRPCAPI grpc_stream_compression_algorithm grpc_stream_compression_algorithm_for_level(
  ...
  switch (level) {
    ...
    case GRPC_STREAM_COMPRESS_LEVEL_HIGH:
      if (GPR_BITGET(accepted_stream_encodings, GRPC_STREAM_COMPRESS_GZIP) == 1) {
